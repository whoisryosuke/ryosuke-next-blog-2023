---
title: Intro to Visualizing Audio Programming
date: "2024-03-12"
section: blog
tags: ["blender", "ui", "tips"]
cover_image: "./how-i-made-the-render-buddy-blender-plugin.png"
---

import dynamic from "next/dynamic";
export const BlogMDXWrapper = dynamic(
  () => import("@components/dom/BlogMDXWrapper/BlogMDXWrapper"),
  { ssr: false }
);
export default function Layout({children}) {
  const frontmatter = {
    title: "Intro to Visualizing Audio Programming",
    date: "2024-03-12",
    section: "blog",
    tags: ["blender", "ui", "tips"],
    cover_image: "./how-i-made-the-render-buddy-blender-plugin.png",
  }
  return <BlogMDXWrapper frontMatter={frontmatter} slug="test-mdx">{children}</BlogMDXWrapper>;

}

export const WaveformExample = dynamic(
  () => import("@components/viz/2024/WaveformExample"),
  { ssr: false }
);
export const CircleWaveformExample = dynamic(
  () => import("@components/viz/2024/CircleWaveformExample"),
  { ssr: false }
);
export const ControlledExample = dynamic(
  () => import("@components/viz/2024/ControlledExample"),
  { ssr: false }
);
 
# MIDI

<WaveformExample />

# Another heading

<CircleWaveformExample />

# Controlled Example

<ControlledExample />
