...
Afterwards though, you end up with a few modules, but most of the code is in one giant file. In a real application, you’d probably split up a lot of the functionality happening into reusable modules. I started to ask myself: how would I render multiple models with different textures — or an _entire scene_?
...
TLDR?: [Here’s the final code.](https://github.com/whoisryosuke/wgpu-hello-world/tree/ecs-setup)
...
Before I set off on the journey of refactoring, I looked for other wgpu projects to see how they structured their app architecture. I found 2 great examples that I primarily referenced - both with “b” names funnily enough: [baryon](https://github.com/kvark/baryon/) and [bevy](https://bevyengine.org/).

**[baryon](https://github.com/kvark/baryon)** is a lightweight toy renderer for prototyping 3D applications in Rust. It uses wgpu to render 3D elements, hecs for an ECS system (to make a scene with “entities” like 3D models), and winit for handling cross-platform window management (just like [the wgpu tutorial](https://sotrh.github.io/learn-wgpu/)). It also allows for setting different “render passes”, like a Phong (”cartoony”/fast) vs PBR (”realistic”) pass.

Bevy is an entire game engine for creating 2D or 3D games in Rust. It uses wgpu for rendering everything, and a few other dependencies - [mostly their own crates](https://github.com/bevyengine/bevy/tree/main/crates). It’s not “fully featured” like Unity or Godot since it’s so early — but it’s jam-packed with a lot of great functionality (like most recently — [compute shaders](https://www.youtube.com/watch?v=v256-eRzak4)).
...
The best place to start is [the library’s simplest example](https://github.com/kvark/baryon/blob/main/examples/scene.rs) to understand the shape of the API and kind of functions we’ll see run.
...
- We create a [“render pass”](https://developer.arm.com/documentation/102479/0100/How-Render-Passes-Work) - in this case a Phong pass [(L#42)](https://github.com/kvark/baryon/blob/main/examples/scene.rs#L42)
...
That’s it! Not too bad. But there’s a bit happening behind the scenes to make this code look so short and elegant. You can also see some _similar_ modules from the wgpu tutorial, like the `Camera`.
...
This worked great, but we get left with our event loop code (`window.event_loop`) outside the `Window` struct. And there’s [a lot of boilerplate code happening](https://github.com/whoisryosuke/wgpu-hello-world/blob/ebff432b5e9c9c9143bb3f267043e95000d333c9/src/lib.rs#L581-L632) that could be moved into the `Window` struct. So let’s do that ([commit for reference](https://github.com/whoisryosuke/wgpu-hello-world/commit/daa6413b6581be01ddf3096a104cff303ae73ad5)).
...
Here’s [the full commit for reference.](https://github.com/whoisryosuke/wgpu-hello-world/commit/76cfcd50a42a28ee68ee6a6ab9ff164a153a7213)
...
A [render pass](https://docs.rs/wgpu/0.12.0/wgpu/struct.RenderPass.html) usually represents a single “pass” of the renderer, using a specific rendering pipeline. [This article does a great job of breaking it down.](https://developer.arm.com/documentation/102479/0100/How-Render-Passes-Work)

In most 3D engines, there are often several render passes that combine or “composite” into the final image. You can see [in the Unreal Engine documentation](https://docs.unrealengine.com/4.27/en-US/AnimatingObjects/Sequencer/HighQualityMediaExport/RenderSettings/RenderPasses/) that they do separate render passes for **lighting**, **shadows**, **reflections**, and the **unlit models**. Or in [the Godot docs](https://docs.godotengine.org/en/stable/tutorials/shaders/custom_postprocessing.html), you can see post processing as a render pass.
...
It gets even more complex when you go into the parts of a render pass, [like depth stencils](https://sotrh.github.io/learn-wgpu/beginner/tutorial8-depth/#sorting-from-back-to-front), but for now we’ll focus on the high level.

### The render pass came from inside the house

**But let’s take a few steps back**. During the wgpu tutorial, we actually created 2 separate render passes _whether you realized or not_.
...
In this example I’ll focus on creating [a “Phong” pass](https://en.wikipedia.org/wiki/Phong_shading). Later you could create [a PBR (physically based rendering) pass.](https://github.com/kvark/baryon/blob/main/src/pass/real.rs)
...
To start, I basically took all the relevant initialization code and [shoved it in a new struct](https://github.com/whoisryosuke/wgpu-hello-world/blob/76cfcd50a42a28ee68ee6a6ab9ff164a153a7213/src/pass/phong.rs) called `PhongPass`. Inside each pass struct we’d store:
...
It’s a little confusing in our app though, because we setup instancing for models, so our “local” uniform data (like position) is stored there instead. But [we do have “local” uniforms](https://github.com/whoisryosuke/wgpu-hello-world/blob/98d83d5a55cfa2b0114a4768a9f2c3f32c0113db/src/shader.wgsl#L86-L89) in the form of our texture data.
...
> _Note: In this commit/version, I only have 1 uniform buffer, because we don’t have multiple objects yet. We’ll do something special later to handle that._
...
And then in [our draw method and render pass](https://github.com/whoisryosuke/wgpu-hello-world/blob/e461c5e590ec586262ee112986a71a0859a334f3/src/pass/phong.rs#L463-L491), we can set the globals first, then the locals. Later, we’ll loop over all objects in the scene and create bind groups and buffers for each object.
...
[🎥 See a video on Twitter here](https://twitter.com/whoisryosuke/status/1592692904996151296?s=20&t=dkT7yZgvMlPDkVs1Y_P4WA)
...
[🎥 See a video on Twitter here](https://twitter.com/whoisryosuke/status/1593409473393045504?s=20&t=dkT7yZgvMlPDkVs1Y_P4WA)
...
Want to share your progress or ask any questions? Feel free to reach out to me on [Twitter](https://twitter.com/whoisryosuke) or [Mastodon](https://mastodon.gamedev.place/@whoisryosuke). Also make sure to check out the [Rust Game Development](https://gamedev.rs/) group’s [Discord channel](https://discord.gg/j6QJsMd), there’s lots of cool people on there that work on wgpu too. And thanks again to [the author of Baryon](https://github.com/kvark) for such a great resource on advanced wgpu architecture.
