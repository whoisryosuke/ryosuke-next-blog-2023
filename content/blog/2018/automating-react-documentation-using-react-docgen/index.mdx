...
Also imports the documentation as a GraphQL endpoint, allowing us to query the data -- instead of hard-importing a _(potentially giant)_ JSON file -- or using the `gatsby-transformer-json` plugin (which doesn't format the data for GraphQL as well as the specific react-docgen plugin).

![Gatsby Documentation Starter using gatsby-transformer-react-docgen](./screenshot.png)
...
There are plenty of out-of-the-box solutions out there, but it's always beneficial understanding how their work (demystifying the _magic_ ✨), particularly if you're looking to create something new and fresh _(frameworks only go so far)_.
