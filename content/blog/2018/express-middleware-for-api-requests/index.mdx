...
Add an async before the function that renders your route -- _because don't stress it, it's easy to forget that you're working **inside** a function_. Now you just slap an await on Promise you want to fetch before page load.
...
const credentials = {
  ...
  headers: {
    "Content-Type": "application/json",
    Authorization:
      "Basic " + btoa(process.env.API_USER + ":" + process.env.API_VENDOR),
  },
};
...
async function fetchApi(endpoint) {
  return await fetch(process.env.API_URL + endpoint, credentials).then((r) =>
    r.json()
  );
}
...
function getData(endpoint) {
  return (req, res, next) => {
    ...
    const fetchData = async () => {
      await fetchApi(endpoint).then((data) => {
