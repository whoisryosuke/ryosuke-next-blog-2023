...
With the power of abstract classes in OOP, we can create a kind of _base_ class to _extend_ the functionality of our specific endpoint's test class. Let's create a new test called `tests/CrudTest.php`:
...
> Use _states_ to add extra data or modify existing params in your fake Model entry. `factory(YourModelName::class)->state('product');`Works great if you need to have control over a model's specific parameter(s), like setting a user as an admin or customer. In this case, we set the post's category.
...
It really is this **simple** to test your API _(if it's a simple CRUD API)._
...
Testing is easy, setting up the proper conditions becomes the true _test_.
