...
It’s been a while since I wrote and released [**react-unified-input**](https://github.com/whoisryosuke/react-unified-input), and I wanted to share the process behind creating it and demystify focus management and spatial navigation in modern applications.
...
That’s why I created **[react-unified-input](https://github.com/whoisryosuke/react-unified-input)** - a React library that handled both in a simple to use package.
...
![The MDN web page footer with the About link element focused using tab based navigation. Focus appears as a solid white rounded outline in the Chrome browser.](Focus%20Management%20and%20Spatial%20Navigation%20in%20React%2054af838ca0594f5486924681e535c6cd/Untitled.png)
...
![The start screen for Assassin’s Creed Odyssey with a circle representing the cursor which is hovered over a menu item. The menu item is lighter than others, and has a white stroke around it. [Courtesy of the Game UI Database.](https://www.gameuidatabase.com/gameData.php?id=536#&gid=1&pid=2)](Focus%20Management%20and%20Spatial%20Navigation%20in%20React%2054af838ca0594f5486924681e535c6cd/Untitled%201.png)
...
![A diagram for spatial navigation in a 2D space. A 2D graph with X and Y axis going from 0 to 4 on each. A player sprite is positioned at the coordinates 1,1. Above the sprite is an arrow representing movement up, and a circle shows where the sprite will end up at 3,1. [Thanks for Kenney for the sprite.](https://kenney.nl/assets/rpg-urban-pack)](<Focus%20Management%20and%20Spatial%20Navigation%20in%20React%2054af838ca0594f5486924681e535c6cd/Spatial_Navigation_in_a_2D_Space_(1).png>)

In our case, we’re talking about navigating focus elements - so it’d look more like this:

![Diagram for focus management using spatial navigation. An button labeled Add to Cart is focused on middle top with an arrow leading down and a label next to arrow with text Input: Down. Below is a horizontal list of 3 buttons, with the arrow leading to the middle button.](<Focus%20Management%20and%20Spatial%20Navigation%20in%20React%2054af838ca0594f5486924681e535c6cd/Input_-_Simple_Focus_Nav_(2).png>)
...
![A slightly more complex example of spatial navigation using focus. The next element that will be focused is offset to the right slightly, while another menu below lines up perfectly with top button.](<Focus%20Management%20and%20Spatial%20Navigation%20in%20React%2054af838ca0594f5486924681e535c6cd/Input_-_Offset_Focus_Nav_(1).png>)

If you’ve done low level game development or UI work, you’re probably familiar with this kind of vector (as in _direction_) math. One of the most basic functions you can find yourself writing is a collision detection “algorithm”. In a game, you may need to check if the player has collided with a set of objects in the scene (like other enemies, hazards or bonuses, etc). You can see an example of this in [my blog post on making Asteroids with Bevy and Rust](https://whoisryosuke.com/blog/2023/making-galaga-in-rust-with-bevy-part-1).
...
1. Page loads, all focusable elements render, and they send a signal to a [Zustand](https://github.com/pmndrs/zustand) data store that they’re focusable. It stores their “focus ID” (randomly generated ID) and their current position. All this happens in the `useFocusable()` hook.
2. User presses down on keyboard to navigate. A `<Keyboard />` component inside the `<InputManager />` component has event listeners that update a shared data store with the current input. It also checks the data store for the current “key map”, just in case the bindings change (and down isn’t “down” anymore). We map all input devices to a common “key map”, similar to game engines like Unity or Unreal Engine. So `KeyUp` on keyboard and `002` on Gamepad can both be assigned to the “Up” direction.
3. A `<Navigator />` component sees the input change in the store and runs the navigation algorithm.
4. The algorithm first runs through the focusable elements in the same container and checks if the element is in the direction we’re looking for. Then we run the same process again for anything outside the container if we don’t find anything.
...
The `useFocusable()` hook handles a few things - let’s tackle them bit by bit. You can [see the full hook here](https://github.com/whoisryosuke/react-unified-input/blob/main/src/hooks/useFocusable.ts) for reference.
...
- Cleanup cycles if component is unmounted (so we don’t focus something that’s gone)
...
![Same diagram as earlier for slightly more complex spatial navigation.](<Focus%20Management%20and%20Spatial%20Navigation%20in%20React%2054af838ca0594f5486924681e535c6cd/Input_-_Offset_Focus_Nav_(1).png>)
...
This aligns closely with the concepts I set out with the [input-manager library](https://github.com/whoisryosuke/input-manager) if you’re interested in a platform agnostic solution for game or app development.
...
1. We grab the latest state from the data store. Since callbacks cache the results of outside function calls, we can’t rely on the top-level hook data in this function. Instead, we use the `getState()` method on the hook to force it to provide the latest state when we run it.
2. Because our function uses the DOM to check elements, we need to have an early bail out if we’re running this component server-side (like in a NextJS app). This is easily checked using a `if` statement checking for the global `window` object.
3. We grab the current focus item using the `focusedItem` property which contains the current focused item’s ID. And if we don’t have an item focused we just grab the first item we find and focus it.
4. Then we can finally run through all the focus items. But first if the focus element is inside a focus container, we want to check any other siblings. We also update the position of each item - in case they’ve moved.
5. Then we finally check for collisions! I’ll cover this function separately, but it accepts an array of focus items, a direction, and the current item (to compare against).
6. If we find a collision, we update the focus to the new item using `setFocusedItem()`.
7. If we don’t find a match we do steps 4-5 again for all the other focus items.
...
- It gets [a “cut off coordinate”](https://github.com/NoriginMedia/Norigin-Spatial-Navigation/blob/main/src/SpatialNavigation.ts#L233)
...
- [Sorts the filtered siblings by “priority”?](https://github.com/NoriginMedia/Norigin-Spatial-Navigation/blob/main/src/SpatialNavigation.ts#L424)
  - Based on Mozilla’s Firefox OS for TV [remote control nav algorithm](https://developer.mozilla.org/en-US/docs/Mozilla/Firefox_OS_for_TV/TV_remote_control_navigation#Algorithm_design)
  - Grabs [“ref corners”?](https://github.com/NoriginMedia/Norigin-Spatial-Navigation/blob/main/src/SpatialNavigation.ts#L261)
  ...
  - Uses [Lodash sortBy](https://lodash.com/docs/#sortBy) to sort the siblings
    ...
    - [Checks for “adjacent slice”](https://github.com/NoriginMedia/Norigin-Spatial-Navigation/blob/main/src/SpatialNavigation.ts#L357)
    ...
    - It calculates [“total distance points”](https://github.com/NoriginMedia/Norigin-Spatial-Navigation/blob/main/src/SpatialNavigation.ts#L474) by combining the two axis functions
...
- The [`getNextFocusableChildInDirection()` function](https://github.com/bbc/lrud/blob/master/src/index.ts#L555) is actually very short and simple.
  ...
  - And the `getNextFocusableChild` function works probably like you think it does. It looks through the siblings after the focused item (starting it’s `for` loop after the `node.activeChild.index`). We [check if the node is focusable](https://github.com/bbc/lrud/blob/master/src/index.ts#L1084), we return it back to the previous function (ending the loop).
